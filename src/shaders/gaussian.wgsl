...
fn vs_main(
    ...
    let centre_radius_ndc = splat.centre_radius_ndc; // clip space
    let center_ndc = unpack2x16float(centre_radius_ndc[0]);
    let radius_ndc = unpack2x16float(centre_radius_ndc[1]);
    ...
    let offset_ndc = corners[in_vertex_index] * radius_ndc; // NDC delta

    let a = unpack2x16float(splat.conic_opacity[0]);
    let b = unpack2x16float(splat.conic_opacity[1]);

    let conic = vec3<f32>(a.x, a.y, b.x);
    let opacity = b.y;
